...
const Page = () => {
  ...
  //   fetchUser();
  // }, [fetchPatientProfile, router, session?.user?.id, setActiveItem, setUser, status, user]);
  ...
  //     }
  //   }
  // }, [setActiveItem, user]); // Now depends on 'user' from Zustand
  ...
  }, [user?.id, setProfile]); // Keep these dependencies as they are correct


  const fetchAppointments = React.useCallback(async (id: string) => {
    setLoading(true);
    try {
  ...
  // useEffect to fetch initial user data
  useEffect(() => {
    const fetchUser = async () => {
      if (status === "loading" || !session?.user?.id) {
        return;
      ...
      try {
        ...
        console.log("Response from API (fetchUser):", res.data);
        const fetchedUser = res.data;
        if (fetchedUser) {
          setUser(fetchedUser); // This will trigger the next useEffect for profile fetching
        }
        if (fetchedUser && !fetchedUser.profileSetup) {
          toast("Please complete your profile setup to access all features.");
          setActiveItem("profile");
        ...
        setLoading(false); // Set loading to false here after user data is processed
    ...
    fetchUser();
  }, [session?.user?.id, status, setUser, setActiveItem, router]); // Dependencies for initial user fetch

  // useEffect to fetch patient profile once user is available
  useEffect(() => {
    if (user && user.id) { // Only fetch profile if user object and its ID are available
      console.log("User available, fetching patient profile...");
      fetchPatientProfile();
    }
  }, [user, fetchPatientProfile]); // Depend on user and fetchPatientProfile callback
  ...
  }, [profile, fetchAppointments]); // Added fetchAppointments to dependency array if it's a prop or a useCallback
